...
> Q5: 想在手机上跑得动吗？需要把模型算力降到什么程度？

当前仓库只提供面向 Windows 桌面 GPU 的实现（CUDA / TensorRT / DirectML + OBS / Spout2），移动端没有可以直接运行的入口，必须自行移植。若要在手机或平板尝试，需要：

1. 选最轻量的THA模型（`v3 separable` 或 `v4_student`），使用半精度或INT8量化，将输出分辨率压到256~320，关闭眉毛、SR、补帧等附加模型，只保留主模型。
2. 将帧率限制在15~20fps，并开启姿态简化（`simplify>=1`），减少缓存体积，尽量用统一分辨率避免额外resize开销。
3. 推理框架需改为移动端可用的 ONNX Runtime Mobile / TFLite (NNAPI) / CoreML 等，并重写摄像头/音频采集与渲染链路（替换 OBS/Spout2），必要时将生成端放在服务器、手机只做输入/播放。
4. 依据桌面测试数据（RTX 3060 在512p、separable_half可60fps+），折算后移动端想在256p达到15~20fps大致需要 5~10 TOPS 的NPU算力；低于约3 TOPS 通常只能接受 <10fps，实机效果取决于厂商内核实现和驱动。

目前仓库没有提供上述量化/转换脚本或移动端适配代码，能否跑通取决于自行做完裁剪、转换与移动端渲染改造的程度。
